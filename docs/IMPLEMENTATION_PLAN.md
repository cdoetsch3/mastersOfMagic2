...
- **TYPE_EFFECTS §7a** — the banked spell designs. ⚠️ The ORIGINAL 16 were never committed and are lost (verified against full git history, 2026-08-25); §7a now holds the REBUILT bank — ✅ DESIGN-FINAL 2026-08-28: 34 spells in status-set form (named statuses per axis, 1–3 price-point spells each; element-agnostic; polarity + stat derivation as the two engine seams) designed 2026-08-26 across three same-day revisions with Christian (rev 2: Reflect + Meditate, Blight promoted, aux-defense/aux-offense priority lanes, 10%-hit-floor + 90/90-deflect clamps; rev 3: Agony/Torment spell-lane DoTs, Scour flipped to enemy-DoT detonator, Shatter reworked to the 5c no-damage turtle-breaker, Miasma/Ardent/Steadfast renames, Discharge→prio 8 + Overload→prio 9 reclassification ruled for the build batch; rev 4: Death Wish named at 2c, Fester 2c/+4 confirmed by EV pass, Agony to 6/turn, Cleanse 2c + Purify 5c proposed to fill the self-cleanse hole; rev 5: Cleanse/Purify APPROVED, Wither tier 2 approved at 4c/30t — Wither status always −50%, tier 2 buys duration only; rev 6 CLOSED THE BANK: Atrophy confirmed, Agony/Torment bands approved, unlock schedule approved no-notes and mirrored to PROGRESSION §4.2, Fester tempo math recalibrated to turns-not-charges; rev 7 post-close tuning: Fester LOCKED 1c/+3, Agony/Torment ticks 7/5 — the dream line lands at 11.6 dmg/turn, just above Cataclysm cycling, the intended archetype ceiling — and DoT ticks ruled to be DAMAGE (shield-first, deflectable, never miss/crit/proc), with a PvE watch item since enemies never cleanse). **BUILD WAVE (release 4, opened 2026-08-28 at d332e13)** — Phase 1 lanes: charge-cost pips on duel spell tabs ✅ MERGED (cf8b13b: ChargeDots widget, reserved-space law honored, Barrage hollow-ring variable cost, 17 tests); Spells-screen sort/filter/organize ✅ MERGED (bc0ed89: spell_browser.dart kind derivation, resolution-order sections, pinned toolbar for press stability, 36 tests); engine seams ✅ MERGED (10fa355: StatusPolarity on all statuses w/ classification rationale, six effective* getters over statusContributionTo, CombatClamps 10%-floor + 90/90, Discharge→auxOffense 8 — Overload was already 9 per a July ruling; belt flake root-caused and fixed; engine 345→370). **Phase 2 — four content lanes**: stat-stance sets ✅ MERGED (aeb58c7: bank_stances.dart, 10 spells / 5 statuses on the seams, StanceEffect w/ Blind-cleanse pair, parked in Spellbook.stances pending app-lane tooltips/icons; engine 370→401); special stances + sustain ✅ MERGED (ed9ebf9: bank_specials.dart, unified StanceEffect w/ grants list + cleanse rider carrying all 17 stance spells, Steadfast gain-time, Death Wish no-RNG crits, Reflect extended by the manager to EVERY deflect site via _maybeReflect — ticks and Scour packets reflect too, 2 new integration tests; engine 441→476); riders/finisher/cleansers ✅ MERGED (c8493f8: bank_riders.dart, PhaseEffect(AttackBypass) trio, Execute 4c crit<30% through the unified _rollsCrit door — Death Wish/Composure folded in at the merge, one crit door + one counter; Cleanse chosen-status wire payload appended-only-when-non-null; byId now resolves everything — a real desync fixed; Meditate TurnTimed-only; engine 476→522); DoTs + debuff suite ✅ MERGED (dca2c93: bank_dots.dart, 11 spells, _damagePacket as the single damage path — Ignite ticks now deflectable per the tick ruling; Murk = first live StatModifier; per-tick scales by caster power at application; parked in Spellbook.bankDots, Spellbook.everything unifies the park-lists; engine 401→441). ⚠️ Ruling needed from Christian: Discharge still rolls to-hit despite sitting at aux-offense — should it stop missing, like the bank's aux-offense spells? ⚠️ Wave lessons: worktrees cut from stale b434c09 (agents must ff to main first); /tmp/duel.bak collided across two lanes' mutation backups (use worktree-local paths). Known merge hotspots for the manager: spellbook.dart additive blocks ×4, duel.dart crit path (Composure/Death Wish/Execute), deflect path (Divert/Reflect/tick pipeline), Keen/Heavyhand shared ids (Bloodlust lane). **PHASE 2 COMPLETE (2026-08-28): all 34 banked spells are engine-live** — parked in Spellbook.stances/bankSpecials/bankDots/bank pending app-side copy (tooltip_consistency_test gates promotion into [all]); balance sim post-integration: all twelve elements in the 40–60 band; engine suite 370→522 across the wave. **THE ACADEMY (2026-09-10, built before the release-4 deploy at Christian's call)** — the level playing field for testing with friends: everyone duels at level 50, no gear, no belt, no reward (no XP/gold/loot/quest credit — ruled), open to guests via ANONYMOUS Firebase sign-in (⚠️ the Anonymous provider must be enabled in the Firebase console before deploy — no rules change needed, `request.auth != null` already admits it), its own matchmaking queue (tickets/rooms carry `mode: academy|geared`; legacy tickets = geared; a guest joining a room by code ADOPTS the host's mode), and one dedicated Academy loadout per profile (`PlayerProfile.academyPreset`, edited under the Academy chip in the Spellbook — every spell/element open regardless of level, editable anywhere, 10/5 caps by ruling — so when spell unlocks switch on the Academy still opens the whole book). Seams: `lib/game/academy.dart` (`Academy` constants + `duelInputsFor` — the one launch seam overriding level/gear/belt together), `RemoteDuelDriver.academy` coerces the wire's level/gear to 50/none so lockstep can never desync on mode, `LocalAiDriver.levelOverride` for stand-ins, `launchDuel(academy:)` skips `recordDuelResult`, the result card says "nothing gained, nothing lost". Anonymous guests keep the LOCAL profile (`AuthService.signedIn` excludes them; only matchmaking reads the uid). Home tab: "The Academy" button beside Find a duel. Tests: academy_test (inputs seam, default hand resolves, profile round-trip, queue mode filter incl. legacy tickets, driver coercion, AI override) + a Spellbook widget test (level-1 player equips Cataclysm in the Academy loadout; campaign preset untouched).

**PHASE 3 COMPLETE (2026-08-29, done by the manager solo per Christian)**: all 34 spells carry descriptions/icons/tooltip arms and are PROMOTED into Spellbook.all (185c6c4); LadderAi EV heuristics live at the sees-statuses rung — DoTs at 60%-of-face minus refresh waste, Scour/Fester/Shatter/Dispel/Cleanse/Purify/Mending all priced, plus structural fixes (deterministic attack sorts, per-loadout charge targets, build-cap = biggest castable) and a re-spread blunder gradient (2:0.44) since cheap-attack spam got genuinely stronger; ladder climbs on every rung; balance sim with the bank playable: all twelve elements 46.3–52.8% (tightest band recorded); regrow/healOverTime catalog+fx gap closed; Cleanse pick-a-debuff dialog shipped (CleansePickerDialog, three-way contract: pick / auto / dismissal-cancels, ≤1 debuff skips the dialog); PROGRESSION §4.2 folded into the §4 master table (⚔ rows), mirror retired. Suites: engine 523, app 1615. Remaining before deploy: Christian's in-browser pass (pips, spellbook, picker, tooltips) and his playtest of the bank; status-fx placeholder art refinement stays open for a future HUD polish pass. Playtest note #1 landed 2026-08-29: the Spells screen's Aux shelf split into Aux-self / Aux-offense (spell_browser.dart derives on isHarmful-without-isOffensive; priorityLabel + detail chip aligned). Playtest note #2 (Spellbook UX batch, approved with two amendments): [A] effect line on tiles via shared spellEffectLine, [B] kind stripe + sections survive cost/search (only a kind chip flattens), [C] locked-preview dim + "Lv N" + Locked toggle reading a NEW Progression.plannedUnlockLevel (§4 table) — ⚠️ the old spellUnlockLevel table (enemy rosters test against it) DISAGREES with the ruled schedule; reconciling = enemy-kit changes, needs a ruling — [D] charge dots top-right (per note), [E] pinned IN LOADOUT tray w/ tap-to-unequip, [F] search + per-chip counts (padded to 2 figure-spaces, tabular numerals — the press-stability test caught count-width drift), [G] cost row left-aligned. Playtest note #3: tile subtext (effect line + gold Lv prefix) REMOVED — hover carries it; icon/name/dots enlarged; dots top-right. Note #4: GROUPING is the primary axis (Group: None / Spell Type / Charge Cost; Sort: Unlock Level [default] / Name / Charge Cost, in a fourth toolbar row; sectionSpells in spell_browser.dart; X-cost files by minimum in both chip and header). Note #5: the Material tooltip on tiles replaced by HoverCard (lib/ui/hover_card.dart, OverlayPortal, 350ms rest, mouse-only, pointer-transparent card) showing the ⓘ dialog's own SpellDetailCard (extracted, showDone/note params) — one card, not two descriptions. ✅ Both post-wave rulings landed 2026-08-29: Discharge no longer rolls to-hit (deals no damage — Blind can't fizzle it anymore, old pin reversed with the ruling cited); park-lists consolidated behind ONE canonical `Spellbook.banked` (the four lane sublists remain as internal groupings; `everything = [...all, ...banked]`; the app lane promotes `banked` into `all` when copy lands)., plus a DRAFT unlock schedule against PROGRESSION §4, the counter-web, a build-time test ledger, and the open decisions that gate building.
