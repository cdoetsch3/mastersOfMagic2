...
- **TYPE_EFFECTS §7a** — the banked spell designs. ⚠️ The ORIGINAL 16 were never committed and are lost (verified against full git history, 2026-08-25); §7a now holds the REBUILT bank — 28 spells in status-set form (named statuses per axis, 1–3 price-point spells each; element-agnostic; polarity + stat derivation as the two engine seams) designed 2026-08-26, rev 2 same day after Christian's 16-note pass (Reflect + Meditate added, Blight promoted to its own status, priorities split into aux-defense/aux-offense lanes, 10%-hit-floor + 90/90-deflect clamps, DRAFT unlock placements against PROGRESSION §4), with the counter-web and the open decisions that gate building them.
