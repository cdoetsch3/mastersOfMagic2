...
- **TYPE_EFFECTS §7a** — the banked spell designs. ⚠️ The ORIGINAL 16 were never committed and are lost (verified against full git history, 2026-08-25); §7a now holds the REBUILT bank — 31 spells in status-set form (named statuses per axis, 1–3 price-point spells each; element-agnostic; polarity + stat derivation as the two engine seams) designed 2026-08-26 across three same-day revisions with Christian (rev 2: Reflect + Meditate, Blight promoted, aux-defense/aux-offense priority lanes, 10%-hit-floor + 90/90-deflect clamps; rev 3: Rend/Torment spell-lane DoTs, Scour flipped to enemy-DoT detonator, Shatter reworked to the 5c no-damage turtle-breaker, Deathwish 📝, Miasma/Ardent/Steadfast renames, Discharge→prio 8 + Overload→prio 9 reclassification ruled for the build batch), plus a DRAFT unlock schedule against PROGRESSION §4, the counter-web, a build-time test ledger, and the open decisions that gate building.
