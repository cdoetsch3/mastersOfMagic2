...
void main() {
  group('shieldRings splits a total into 100-point rings', () {
    ...
    test('the rings always sum back to the total', () {
      for (var total = 1; total <= 400; total++) {
        expect(shieldRings(total).fold(0, (a, b) => a + b), total,
            reason: 'total $total');
      }
    });

    test('120 taking a 40-point hit drops the small ring and thins the big one',
        () {
  ...
  group('the aura paints without error across the whole range', () {
    ...
    for (final (label, colour, remaining, barrier) in <(String, Color?, int, int)>[
  ...
  test('stroke scales with the sprite so it reads the same on any screen', () {
    ...
    expect(shieldPxPerPoint * 75, closeTo(15.0, 0.01), reason: 'Sanctuary ~15px');
    expect(shieldPxPerPoint * shieldPointsPerRing, 20.0,
